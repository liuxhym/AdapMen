...
def log_and_write(writer, log_infos: List, global_step: int):
    for idx, (name, value) in enumerate(log_infos):
        logkv('{}.'.format(idx) + name.split('/')[-1], value)
        if writer is not None and name.find('/') > -1:
            writer.add_scalar(name, value, global_step=global_step)
    dumpkvs()


def init_logging(cfg, hparam_dict, info_str=''):
    import datetime
    start_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    if 'btq' in cfg.keys():
        log_dir = os.path.join(os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name), cfg.algo) + f'--teacher-{cfg.teacher}--{start_time}' + info_str + '/train'
        save_dir = os.path.join(os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name), cfg.algo) + f'--teacher-{cfg.teacher}--{start_time}' + info_str + '/save'
    elif 'dagger' in cfg.keys():
        log_dir = os.path.join(os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name), cfg.algo) + f'--num_traj-{cfg.env.num_traj}--initialize-{cfg.dagger.bc_step}--{start_time}'  + info_str + '/train'
        save_dir = os.path.join(os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name), cfg.algo) + f'--num_traj-{cfg.env.num_traj}--initialize-{cfg.dagger.bc_step}--{start_time}' + info_str + '/save'
    elif 'human' in cfg.keys():
        log_dir = os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name) + f'--num_traj-{cfg.env.num_traj}--initialize-{cfg.dagger.start_training_timesteps}--{start_time}' + info_str + '/train'
        save_dir = os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name) + f'--num_traj-{cfg.env.num_traj}--initialize-{cfg.dagger.start_training_timesteps}--{start_time}' + info_str + '/save'
    else:
        log_dir = os.path.join(os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name), cfg.algo) + f'--{start_time}' + info_str + '/train'
        save_dir = os.path.join(os.path.join(os.path.join('logs', cfg.env.env_name), cfg.task_name), cfg.algo) + f'--{start_time}' + info_str + '/save'
    ...
    #         hparam_dict[k] = torch.as_tensor(v)

    # exp, ssi, sei = hparams(hparam_dict, {}, None)
    # writer.file_writer.add_summary(exp)
    # writer.file_writer.add_summary(ssi)
    # writer.file_writer.add_summary(sei)

    configure(log_dir, None, False, None)
    info('Hyperparms:')
    for key, value in hparam_dict.items():
        log('{:35s}: {}'.format(key, value))

    return writer, log_dir, save_dir
