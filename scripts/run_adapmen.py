...
from adapmen.env import create_il_env, METADRIVE_ENVS, ATARI_ENVS, EXPERT_PERFORMANCE
...
def main(config_path, gpu, seed, info):
    ...
    if env.discrete:
        ...
        actor = Actor(env.observation_space, env.action_space.shape[0], cfg.sac.actor_network_params)   # "actor_network_params": [("mlp", 256), ("mlp", 256)]
        # actor = NewActor(259, env.action_space.shape[0], cfg.sac.actor_network_params)  # "actor_network_params": [256, 256],
        actor.to(device)
    ...
    while total_timesteps < cfg.btq.max_timesteps:
        ...
        if done: 
            episode_num += 1
            if total_timesteps > cfg.btq.samplesteps + 2*cfg.btq.update_p_freq:
                log_infos = [('perf/train_return', episode_return)]
                log_and_write(writer, log_infos, global_step = total_timesteps)
                log_infos = [('perf/train_intervetion_ret', episode_return)]
                log_and_write(writer, log_infos, global_step = expert_intervention)
        ...
        if cfg.env.env_name in METADRIVE_ENVS and episode_timesteps > 1000:    
            done = True
        elif cfg.env.env_name in ATARI_ENVS and episode_timesteps > 2000:
            done = True
        
        # if total_timesteps == cfg.btq.samplesteps:
        #     old_expert_intervention = expert_intervention

        if total_timesteps > cfg.btq.samplesteps and total_timesteps % cfg.btq.update_p_freq == 0:   
            batch_actor_loss = 0
            for _ in range(cfg.btq.updates_per_step):
                policy_buffer_gen = policy_buffer.get_batch_generator_inf(cfg.btq.batch_size)
                infos = imitator.update(next(policy_buffer_gen))
            ...
            expert_intervention_prob = (expert_intervention - totsteps_intervention_map[total_timesteps-( cfg.btq.smooth * cfg.btq.update_p_freq)]) / (5*cfg.btq.update_p_freq)
